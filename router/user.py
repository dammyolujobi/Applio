...
@router.post("/create_user")                          # ✅ POST, not GET
async def create_user(user: User = Depends()):
    check_email = user_collection.find_one({"email": user.email})  # ✅ dict query
    ...
    return JSONResponse(
        {"content": "Successfully Registered", "access_token": access_token},
        status_code=200,
    )
...
async def login(login: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    if not confirmed_email:
        return JSONResponse(
            content={"detail": "Invalid Email or Password"}, status_code=400
        )
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])  # ✅ list
        return payload["sub"]
    except JWTError:
